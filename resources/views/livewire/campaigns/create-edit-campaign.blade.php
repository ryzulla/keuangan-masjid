<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base" style="color:#161e2d;">{{ $pageTitle }}</h2>
    </x-slot>
    ...
    <div class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8 px-4 space-y-6">
        ...
        <div class="flex items-center gap-3">
            <button wire:click="cancel" class="flex items-center gap-1.5 text-sm transition-colors" style="color:#5c6368;"
            ...
            <span class="text-sm font-semibold" style="color:#161e2d;">{{ $pageTitle }}</span>
        </div>
        ...
        <div class="rounded-2xl p-6" style="background:linear-gradient(135deg,#ffffff 0%,#ffffff 62%);border:1px solid rgba(21,99,223,0.35);">
            <div class="flex items-center gap-3">
                <div>
                    <h1 class="text-xl font-bold" style="color:#161e2d;font-family:'Manrope',serif;">{{ $pageTitle }}</h1>
                    <p class="text-xs mt-1" style="color:#161e2d;">
                ...
                <span class="ml-auto px-3 py-1 rounded-full text-xs font-semibold"
                    style="{{ $organization_type === 'perumahan' ? 'background:rgba(21,99,223,0.15);color:#161e2d;border:1px solid rgba(21,99,223,0.3);' : 'background:rgba(18,128,92,0.12);color:#12805c;border:1px solid rgba(18,128,92,0.25);' }}">
        ...
        <form wire:submit="store" class="space-y-6">
            ...
            <div class="rounded-2xl p-6 space-y-5" style="background:#ffffff;border:1px solid #f7f7f7;">
                <h2 class="text-sm font-semibold uppercase tracking-wider" style="color:#161e2d;">Informasi Dasar</h2>

                {{-- Nama --}}
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Nama Program <span style="color:#c0453b;">*</span></label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {{-- Jenis Program --}}
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Jenis Program <span style="color:#c0453b;">*</span></label>
                    ...
                    {{-- Sumber Dana --}}
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Sumber Dana Awal</label>
                    ...
                    {{-- Status --}}
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Status <span style="color:#c0453b;">*</span></label>
                    ...
                    {{-- Lokasi --}}
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Lokasi</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Target Donasi (Rp)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Tanggal Mulai <span style="color:#c0453b;">*</span></label>
                        <input type="date" wire:model="start_date"
                            class="w-full px-4 py-2.5 text-sm rounded-xl outline-none"
                            style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);color:#161e2d;color-scheme:dark;"
                            onfocus="this.style.borderColor='#1563df'" onblur="this.style.borderColor='#e4e4e4'">
                        @error('start_date')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">Tanggal Selesai</label>
                ...
                {{-- URL Video --}}
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:#5c6368;">URL Video (YouTube — Opsional)</label>
            ...
            <div class="rounded-2xl p-6 space-y-4" style="background:#ffffff;border:1px solid #f7f7f7;">
                <h2 class="text-sm font-semibold uppercase tracking-wider" style="color:#161e2d;">Gambar Utama</h2>

                <div class="flex flex-col sm:flex-row gap-5 items-start">
                    {{-- Preview --}}
                    <div class="shrink-0">
                        @if($image && !$errors->has('image'))
                            <img src="{{ $image->temporaryUrl() }}" class="w-32 h-24 object-cover rounded-xl" style="border:1px solid #e4e4e4;">
                        @elseif($existingImage)
                            <div class="relative group">
                                <img src="{{ Storage::url($existingImage) }}" class="w-32 h-24 object-cover rounded-xl" style="border:1px solid #e4e4e4;">
                                <button type="button" wire:click="removeImage"
                                    ...
                                    style="background:#c0453b;color:#161e2d;">✕</button>
                            </div>
                        @else
                            <div class="w-32 h-24 rounded-xl flex items-center justify-center" style="background:#ffffff;border:1px dashed #e4e4e4;">
                                <svg class="w-8 h-8 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14"/></svg>
                            </div>
                        @endif
                    </div>
                    <div class="flex-1">
                        ...
                        <div wire:loading wire:target="image" class="text-xs mt-1" style="color:#161e2d;">Mengunggah...</div>
                        @error('image')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                        <p class="text-xs mt-2" style="color:#a3abb0;">JPG, PNG, WebP — maks. 2 MB. Rasio 16:9 direkomendasikan.</p>
                    </div>
                </div>
            </div>

            {{-- ── SECTION 3: Deskripsi Singkat ── --}}
            <div class="rounded-2xl p-6 space-y-3" style="background:#ffffff;border:1px solid #f7f7f7;">
                <h2 class="text-sm font-semibold uppercase tracking-wider" style="color:#161e2d;">Deskripsi Singkat</h2>
                <p class="text-xs" style="color:#a3abb0;">Ringkasan 1–2 kalimat yang tampil di kartu program (teks biasa).</p>
                <textarea wire:model="description" rows="3" placeholder="Ringkasan singkat program untuk ditampilkan di listing..."
                    ...
                    style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);color:#161e2d;min-height:80px;"
            ...
            <div class="rounded-2xl p-6 space-y-3" style="background:#ffffff;border:1px solid #f7f7f7;">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-semibold uppercase tracking-wider" style="color:#161e2d;">Konten Lengkap</h2>
                        <p class="text-xs mt-0.5" style="color:#a3abb0;">Opsional — deskripsi panjang seperti artikel/blog dengan format teks lengkap.</p>
                    </div>
                    <span class="px-2.5 py-1 rounded-lg text-xs font-medium" style="background:rgba(21,99,223,0.1);color:#161e2d;border:1px solid rgba(21,99,223,0.2);">Opsional</span>
                </div>

                {{-- CKEditor container: wire:ignore prevents Livewire from touching the DOM inside --}}
                <div wire:ignore id="ck-editor-wrapper">
                    <div style="border:1px solid #e4e4e4;border-radius:0.75rem;overflow:hidden;min-height:300px;">
            ...
            <div class="rounded-2xl p-6 space-y-4" style="background:#ffffff;border:1px solid #f7f7f7;">
                <div class="flex items-center justify-between">
                    <h2 class="text-sm font-semibold uppercase tracking-wider" style="color:#161e2d;">Galeri Foto ({{ $existingPhotos->count() }})</h2>
                    <p class="text-xs" style="color:#a3abb0;">Foto baru dapat ditambah dari halaman Detail Program.</p>
                </div>
                <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                    ...
                    <div class="relative group rounded-lg overflow-hidden" style="aspect-ratio:1;" wire:key="gphoto-{{ $photo->id }}">
                        ...
                        <button type="button" wire:click="deleteGalleryPhoto({{ $photo->id }})" wire:confirm="Hapus foto ini?"
                            ...
                            style="background:rgba(192,69,59,0.8);color:#161e2d;">✕</button>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between gap-3 pt-2 pb-8">
                <button type="button" wire:click="cancel"
                    ...
                    style="background:#ffffff;color:#161e2d;border:1px solid #e4e4e4;"
    ...
    @push('styles')
    <style>
        .ck.ck-editor__main>.ck-editor__editable { background:#ffffff !important; color:#161e2d !important; min-height:280px; border:none !important; border-top:1px solid #e4e4e4 !important; }
        .ck.ck-toolbar { background:#ffffff !important; border-color:#e4e4e4 !important; }
        .ck.ck-toolbar .ck-toolbar__separator { background:#e4e4e4 !important; }
        .ck.ck-button { color:#161e2d !important; }
        .ck.ck-button:hover,.ck.ck-button.ck-on { background:#e4e4e4 !important; color:#161e2d !important; }
        .ck.ck-editor__editable.ck-focused { border:none !important; box-shadow:none !important; }
        .ck-content h2,.ck-content h3 { color:#161e2d; }
        .ck-content a { color:#161e2d; }
        .ck.ck-list { background:#ffffff !important; border-color:#e4e4e4 !important; }
        .ck.ck-list__item .ck-button:hover { background:#ffffff !important; }
        .ck.ck-balloon-panel { background:#ffffff !important; border-color:#e4e4e4 !important; }
        .ck.ck-dropdown__panel { background:#ffffff !important; border-color:#e4e4e4 !important; }
        .ck.ck-input-text { background:#ffffff !important; border-color:#e4e4e4 !important; color:#161e2d !important; }
    </style>
    @endpush
