<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base truncate" style="color:#161e2d;">{{ $campaign->name }}</h2>
    </x-slot>
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-6">
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold"
                            style="{{ $campaign->status === 'active' ? 'background:rgba(18,128,92,0.2);color:#12805c;border:1px solid rgba(18,128,92,0.4);' : ($campaign->status === 'completed' ? 'background:rgba(21,99,223,0.2);color:#161e2d;border:1px solid rgba(21,99,223,0.4);' : 'background:rgba(192,69,59,0.2);color:#c0453b;border:1px solid rgba(192,69,59,0.4);') }}">
                            {{ $campaign->status === 'active' ? 'Aktif' : ($campaign->status === 'completed' ? 'Selesai' : 'Dibatalkan') }}
                        </span>
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold"
                            style="{{ $campaign->organization_type === 'perumahan' ? 'background:rgba(21,99,223,0.2);color:#161e2d;border:1px solid rgba(21,99,223,0.4);' : 'background:rgba(18,128,92,0.15);color:#12805c;border:1px solid rgba(18,128,92,0.3);' }}">
        ...
        @else
            <div class="rounded-2xl p-6" style="background:#f7f7f7;border:1px solid rgba(21,99,223,0.35);">
                ...
                @if($campaign->description)
                    <p class="mt-2 text-sm" style="color:#161e2d;">{{ $campaign->description }}</p>
                @endif
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            @foreach($campaign->photos as $photo)
                                <div class="relative group rounded-xl overflow-hidden cursor-pointer"
                                    ...
                                    <button wire:click.stop="deletePhoto({{ $photo->id }})"
                                            ...
                                            style="background:rgba(192,69,59,0.9);color:#161e2d;">✕</button>
                                    @endcan
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="space-y-4">
                ...
                <div class="rounded-2xl p-4" style="background:#ffffff;border:1px solid #e4e4e4;">
                    <div class="space-y-2 text-sm">
                        ...
                        @if($campaign->sourceAccount)
                            <div class="flex justify-between">
                                <span style="color:#a3abb0;">Sumber Dana</span>
                                <span class="font-medium" style="color:#161e2d;">{{ $campaign->sourceAccount->name }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                {{-- Admin actions --}}
                @can('manage-transactions')
                <div class="rounded-2xl p-4 space-y-2" style="background:#ffffff;border:1px solid #e4e4e4;">
                    <p class="text-xs font-semibold uppercase tracking-wider mb-3" style="color:#a3abb0;">Kelola Program</p>
                    ...
                    <button wire:click="openGalleryUpload()"
                        class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium w-full transition-colors"
                        style="background:#ffffff;color:#5c6368;border:1px solid #e4e4e4;"
        ...
        <div class="space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <h2 class="text-lg font-bold" style="color:#161e2d;font-family:'Manrope',serif;">
                    Daftar Donasi
                    <span class="ml-2 text-sm font-normal" style="color:#a3abb0;">({{ $stats['count_uang'] + $stats['count_barang'] }} total)</span>
                </h2>
                @can('manage-transactions')
            ...
            <div class="flex flex-wrap gap-2">
                <button wire:click="$set('filterDonationForm', '')"
                    ...
                    @if($filterDonationForm !== '') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
                    Semua
                </button>
                <button wire:click="$set('filterDonationForm', 'uang')"
                    ...
                    @if($filterDonationForm !== 'uang') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
                ...
                <button wire:click="$set('filterDonationForm', 'barang')"
                    ...
                    @if($filterDonationForm !== 'barang') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
                ...
                <button wire:click="$set('filterDonorType', '')"
                    ...
                    @if($filterDonorType !== '') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
                    Semua Asal
                </button>
                <button wire:click="$set('filterDonorType', 'warga')"
                    ...
                    @if($filterDonorType !== 'warga') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
                ...
                <button wire:click="$set('filterDonorType', 'luaran')"
                    ...
                    @if($filterDonorType !== 'luaran') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" @endif>
            ...
            <div class="rounded-2xl overflow-hidden" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr style="background:#ffffff;border-bottom:1px solid #f7f7f7;">
                                ...
                                <th class="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Nilai / Barang</th>
                                <th class="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider hidden md:table-cell" style="color:#a3abb0;">Foto</th>
                        ...
                        <tbody>
                            @forelse($donations as $donation)
                                <tr style="border-bottom:1px solid #f7f7f7;" wire:key="donation-{{ $donation->id }}"
                                    onmouseover="this.style.backgroundColor='#f7f7f7'" onmouseout="this.style.backgroundColor=''">
                                    <td class="px-4 py-3">
                                        <div class="font-medium text-sm" style="color:#161e2d;">{{ $donation->donor_name }}</div>
                                        @if($donation->resident)
                                            <div class="text-xs mt-0.5" style="color:#a3abb0;">{{ $donation->resident->name }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @else
                                            <div class="text-sm" style="color:#161e2d;">{{ $donation->item_description }}</div>
                                            @if($donation->item_quantity)
                                                <div class="text-xs" style="color:#a3abb0;">{{ $donation->item_quantity }}</div>
                                            @endif
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-3 hidden sm:table-cell text-xs whitespace-nowrap" style="color:#a3abb0;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-14 text-center">
                                        <svg class="w-12 h-12 mx-auto mb-3 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                                        <p class="text-sm" style="color:#a3abb0;">Belum ada donasi untuk program ini.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    ...
                    @forelse($donations as $donation)
                        <div class="p-4 space-y-2" wire:key="donation-mobile-{{ $donation->id }}">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="font-medium text-sm truncate" style="color:#161e2d;">{{ $donation->donor_name }}</div>
                                    @if($donation->resident)
                                        <div class="text-xs mt-0.5" style="color:#a3abb0;">{{ $donation->resident->name }}</div>
                                    @endif
                                    <div class="text-xs mt-0.5" style="color:#a3abb0;">{{ optional($donation->created_at)->format('d M Y') }}</div>
                                </div>
                ...
                @if($donations->hasPages())
                    <div class="px-4 py-3" style="border-top:1px solid #f7f7f7;">{{ $donations->links() }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button wire:click="openAddExpense()"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold shrink-0"
                    style="background:#c0453b;color:#ffffff;"
                    onmouseover="this.style.background='#a3372e'" onmouseout="this.style.background='#c0453b'">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    Tambah Pengeluaran
                </button>
                @endcan
            </div>
            <div class="rounded-2xl" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                @if($expenses->isEmpty())
                ...
                @else
                    <div class="p-4" style="border-bottom:1px solid #e4e4e4;">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="background:#ffffff;border:1px solid #d9d9d9;max-height:92vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:#f7f7f7;border-bottom:1px solid rgba(21,99,223,0.35);">
                <h3 class="font-bold" style="color:#161e2d;font-family:'Manrope',serif;">Tambah Donasi</h3>
                <button wire:click="closeDonationModal()" class="p-1 rounded-lg" style="color:#161e2d;"
            ...
            <form wire:submit="saveDonation" class="overflow-y-auto px-6 py-5 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Tanggal Donasi <span style="color:#c0453b;">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Akun Penerimaan <span style="color:#c0453b;">*</span></label>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-md" style="background:#ffffff;border:1px solid #d9d9d9;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 rounded-t-2xl" style="background:#f7f7f7;border-bottom:1px solid rgba(21,99,223,0.35);">
                <h3 class="font-bold" style="color:#161e2d;font-family:'Manrope',serif;">Tambah Foto Galeri</h3>
                <button wire:click="closeGalleryModal()" class="p-1 rounded-lg" style="color:#161e2d;"
                    onmouseover="this.style.background='rgba(21,99,223,0.1)'" onmouseout="this.style.background=''">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <form wire:submit="saveGalleryPhoto" class="p-6 space-y-4" x-data="{ previewGallery: null }">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color:#5c6368;">Pilih Foto <span style="color:#c0453b;">*</span></label>
                    <div class="rounded-xl p-4 flex flex-col items-center gap-3" style="background:#ffffff;border:2px dashed #d9d9d9;">
                        <svg class="w-10 h-10 opacity-30" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        <input type="file" wire:model="galleryPhoto" accept="image/*"
                               ...
                               class="block w-full text-sm" style="color:#a3abb0;">
                        <div wire:loading wire:target="galleryPhoto" class="text-xs" style="color:#161e2d;">Mengunggah...</div>
                        <img x-show="previewGallery" :src="previewGallery" x-transition
                             class="rounded-xl object-cover w-full" style="max-height:180px;border:1px solid #e4e4e4;">
                    </div>
                    @error('galleryPhoto')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Keterangan Foto (Opsional)</label>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="background:#ffffff;border:1px solid #d9d9d9;max-height:92vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:#f7f7f7;border-bottom:1px solid rgba(21,99,223,0.35);">
                <h3 class="font-bold" style="color:#161e2d;font-family:'Manrope',serif;">Tambah Pengeluaran</h3>
                <button wire:click="closeExpenseModal()" class="p-1 rounded-lg" style="color:#161e2d;"
                    onmouseover="this.style.background='rgba(21,99,223,0.1)'" onmouseout="this.style.background=''">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <form wire:submit="saveExpense" class="overflow-y-auto px-6 py-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Keterangan <span style="color:#c0453b;">*</span></label>
                    <input type="text" wire:model="expenseDescription" placeholder="Deskripsi pengeluaran"
                        style="background:#ffffff;border:1px solid #e4e4e4;color:#161e2d;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;"
                        onfocus="this.style.borderColor='#1563df'" onblur="this.style.borderColor='#e4e4e4'">
                    @error('expenseDescription')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" wire:model="expenseAmount" min="1" step="100" placeholder="0"
                            style="background:#ffffff;border:1px solid #e4e4e4;color:#161e2d;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;"
                            onfocus="this.style.borderColor='#1563df'" onblur="this.style.borderColor='#e4e4e4'">
                        @error('expenseAmount')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Tanggal <span style="color:#c0453b;">*</span></label>
                        <input type="date" wire:model="expenseDate"
                            style="background:#ffffff;border:1px solid #e4e4e4;color:#161e2d;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;color-scheme:dark;"
                            onfocus="this.style.borderColor='#1563df'" onblur="this.style.borderColor='#e4e4e4'">
                        @error('expenseDate')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#5c6368;">Akun <span style="color:#c0453b;">*</span></label>
                    ...
                    @error('expenseAccountId')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                </div>
