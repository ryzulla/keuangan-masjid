<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base" style="color:#161e2d;">Program &amp; Kampanye</h2>
    </x-slot>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 space-y-6">
        ...
        <div class="rounded-2xl p-6" style="background:linear-gradient(135deg,#ffffff 0%,#ffffff 62%);border:1px solid rgba(21,99,223,0.35);">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold" style="color:#161e2d;font-family:'Manrope',serif;">Program &amp; Kampanye</h1>
                    <p class="text-sm mt-1" style="color:#161e2d;">Kelola program donasi dan kegiatan perumahan &amp; DKM</p>
                </div>
        ...
        <div class="flex gap-1 p-1 rounded-xl w-fit" style="background:#ffffff;border:1px solid #e4e4e4;">
            <button wire:click="switchTab('dkm')"
                ...
                style="{{ $activeOrgTab === 'dkm' ? 'background:#1563df;color:#ffffff;' : 'color:#a3abb0;' }}"
                @if($activeOrgTab !== 'dkm') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#a3abb0'" @endif>
                Program DKM Masjid
            </button>
            <button wire:click="switchTab('perumahan')"
                class="px-5 py-2 rounded-lg text-sm font-medium transition-all"
                style="{{ $activeOrgTab === 'perumahan' ? 'background:#1563df;color:#ffffff;' : 'color:#a3abb0;' }}"
                @if($activeOrgTab !== 'perumahan') onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#a3abb0'" @endif>
                Program Perumahan
            </button>
        </div>
        ...
        @if($campaigns->isEmpty())
            <div class="rounded-2xl px-4 py-16 text-center" style="background:rgba(21,99,223,0.03);border:1px dashed rgba(21,99,223,0.2);">
                <svg class="w-14 h-14 mx-auto mb-3 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                <p class="text-sm font-medium" style="color:#a3abb0;">Belum ada program {{ $activeOrgTab === 'perumahan' ? 'Perumahan' : 'DKM' }}.</p>
                <p class="text-xs mt-1 mb-4" style="color:#a3abb0;">Klik "Tambah Program" untuk membuat program baru.</p>
                <a href="{{ route('campaigns.create', ['org' => $activeOrgTab]) }}" wire:navigate
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($campaigns->items() as $campaign)
                    <div class="rounded-2xl overflow-hidden flex flex-col" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);" wire:key="campaign-card-{{ $campaign->id }}">
                        {{-- Thumbnail --}}
                        <div class="relative" style="aspect-ratio:16/9;overflow:hidden;background:#ffffff;">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center" style="background:linear-gradient(135deg,#ffffff,#ffffff);">
                                    <svg class="w-12 h-12 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                </div>
                            @endif
                            <div class="absolute top-2 left-2 flex gap-1.5">
                                <span class="px-2 py-0.5 rounded-full text-xs font-semibold"
                                    style="{{ $campaign->status === 'active' ? 'background:rgba(18,128,92,0.9);color:#161e2d;' : ($campaign->status === 'completed' ? 'background:rgba(21,99,223,0.9);color:#161e2d;' : 'background:rgba(192,69,59,0.9);color:#161e2d;') }}">
                                    {{ $campaign->status === 'active' ? 'Aktif' : ($campaign->status === 'completed' ? 'Selesai' : 'Dibatalkan') }}
                                </span>
                            </div>
                            @if($campaign->photos->count() > 0)
                                <div class="absolute top-2 right-2 px-1.5 py-0.5 rounded-lg text-xs flex items-center gap-1" style="background:rgba(0,0,0,0.1);color:#161e2d;">
                        ...
                        <div class="flex-1 p-4 flex flex-col gap-3">
                            <div>
                                <h3 class="font-bold text-sm leading-tight line-clamp-2" style="color:#161e2d;">{{ $campaign->name }}</h3>
                                @if($campaign->description)
                                    <p class="text-xs mt-1.5 leading-relaxed line-clamp-2" style="color:#a3abb0;">{{ $campaign->description }}</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <div class="w-full rounded-full h-1.5" style="background:#e4e4e4;">
                                    <div class="h-1.5 rounded-full" style="width:{{ $progress }}%;background:{{ $progress >= 100 ? '#12805c' : 'linear-gradient(90deg,#1563df,#1563df)' }};"></div>
                                </div>
                                @if($target > 0)
                                    <div class="text-right text-xs mt-0.5" style="color:#a3abb0;">Target: Rp {{ number_format($target, 0, ',', '.') }}</div>
                                @endif
                            </div>

                            {{-- Meta --}}
                            <div class="flex flex-wrap items-center gap-1.5 text-xs" style="color:#a3abb0;">
            ...
            if (!listenersAttached && window.Livewire && window.Swal) {
                Livewire.on('show-campaign-delete-confirmation', (event) => {
                    ...
                    Swal.fire({
                        ...
                        icon: 'warning', showCancelButton: true, confirmButtonColor: '#1563df',
                ...
                Livewire.on('campaignDeleted', () => Swal.fire({ title: 'Berhasil!', text: 'Program berhasil dihapus.', icon: 'success', timer: 2000, showConfirmButton: false, background: '#ffffff', color: '#161e2d' }));
