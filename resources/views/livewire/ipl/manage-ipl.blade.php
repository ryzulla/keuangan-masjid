<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base" style="color:#111827;">IPL — Iuran Perumahan</h2>
    </x-slot>
    ...
    <div class="py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
        ...
        <div class="rounded-2xl p-6" style="background:linear-gradient(135deg,#ffffff 0%,#ffffff 62%);border:1px solid rgba(16,24,40,0.35);">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h3 class="font-bold text-lg" style="color:#111827;font-family:'IBM Plex Sans',serif;">IPL — Iuran Perumahan</h3>
                    <p class="text-sm mt-1" style="color:#111827;">Kelola iuran bulanan penghuni perumahan</p>
                </div>
        ...
        <div class="rounded-2xl p-4" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
            <div class="flex flex-wrap gap-3 items-center">
                ...
                @if($currentPeriod)
                    <span class="text-xs px-2 py-1 rounded-lg font-medium" style="background:rgba(16,24,40,0.1);color:#111827;border:1px solid rgba(16,24,40,0.2);">
                        {{ $currentPeriod->period_label }}
                    </span>
                @else
                    <span class="text-xs italic" style="color:#98a2b3;">Belum ada periode untuk bulan/tahun ini.</span>
                @endif
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                ...
                <p class="text-xl font-bold mt-1 truncate" style="color:#1d2939;">Rp {{ number_format($summary['total_tagihan'] ?? 0, 0, ',', '.') }}</p>
                <p class="text-xs mt-1" style="color:#98a2b3;">{{ ($summary['jumlah_lunas'] ?? 0) + ($summary['jumlah_belum'] ?? 0) + ($summary['jumlah_sebagian'] ?? 0) }} unit</p>
            </div>
            <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                <p class="text-xs font-medium uppercase tracking-wide" style="color:#98a2b3;">Sudah Terbayar</p>
                <p class="text-xl font-bold mt-1 truncate" style="color:#12805c;">Rp {{ number_format($summary['total_terbayar'] ?? 0, 0, ',', '.') }}</p>
                <p class="text-xs mt-1" style="color:#98a2b3;">{{ $summary['jumlah_lunas'] ?? 0 }} unit lunas</p>
            </div>
            <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                ...
                <p class="text-xs mt-1" style="color:#98a2b3;">{{ $summary['jumlah_belum'] ?? 0 }} belum, {{ $summary['jumlah_sebagian'] ?? 0 }} sebagian</p>
                @if(($summary['total_dibebaskan'] ?? 0) > 0)
                    <p class="text-xs mt-0.5" style="color:#111827;">Dibebaskan: Rp {{ number_format($summary['total_dibebaskan'], 0, ',', '.') }} (non-kas)</p>
                @endif
            </div>
            <div class="rounded-2xl p-5" style="background:rgba(16,24,40,0.05);border:1px solid rgba(16,24,40,0.2);">
                <p class="text-xs font-medium uppercase tracking-wide" style="color:#111827;">Tarif Periode</p>
                <p class="text-sm font-semibold mt-1" style="color:#111827;">Security: Rp {{ number_format($currentPeriod->ipl_security_amount, 0, ',', '.') }}</p>
                <p class="text-sm" style="color:#111827;">Sampah: Rp {{ number_format($currentPeriod->ipl_garbage_amount, 0, ',', '.') }}</p>
                <p class="text-sm" style="color:#111827;">Kas RT: Rp {{ number_format($currentPeriod->ipl_kas_rt_amount, 0, ',', '.') }}</p>
                <button wire:click="openEditPeriod({{ $currentPeriod->id }})" class="text-xs mt-1 hover:underline" style="color:#111827;">Edit Tarif</button>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-3 items-center">
            <select wire:model.live="filterBillingStatus"
                style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);color:#1d2939;border-radius:0.75rem;padding:0.5rem 0.75rem;font-size:0.875rem;outline:none;"
                onfocus="this.style.borderColor='#111827'" onblur="this.style.borderColor='#e4e7ec'">
            ...
            <a href="{{ route('ipl.report') }}" wire:navigate
                ...
                style="background:#f5f6f8;color:#475467;border:1px solid #e4e7ec;"
        ...
        <div class="rounded-2xl overflow-hidden" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr style="background:#ffffff;border-bottom:1px solid #f5f6f8;">
                            ...
                            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider hidden sm:table-cell" style="color:#98a2b3;">Security</th>
                            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider hidden sm:table-cell" style="color:#98a2b3;">Sampah</th>
                            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider hidden sm:table-cell" style="color:#98a2b3;">Kas RT</th>
                            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Total</th>
                            ...
                            <th class="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider hidden md:table-cell" style="color:#98a2b3;">Sisa</th>
                    ...
                    <tbody>
                        @forelse($billings as $billing)
                            <tr style="border-bottom:1px solid #eef0f3;" wire:key="billing-{{ $billing->id }}"
                                ...
                                <td class="px-4 py-3 text-right font-mono text-xs hidden sm:table-cell" style="color:#7c8698;">Rp {{ number_format($billing->ipl_security_amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-mono text-xs hidden sm:table-cell" style="color:#7c8698;">Rp {{ number_format($billing->ipl_garbage_amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-mono text-xs hidden sm:table-cell" style="color:#7c8698;">Rp {{ number_format($billing->ipl_kas_rt_amount, 0, ',', '.') }}</td>
                                ...
                                <td class="px-4 py-3 text-right font-mono text-xs hidden md:table-cell" style="{{ $billing->outstanding > 0 ? 'color:#c0453b;font-weight:600;' : 'color:#98a2b3;' }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex flex-wrap items-center gap-1">
                                        ...
                                        @if($billing->is_waived)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" title="{{ $billing->waiver_reason }}" style="background:rgba(139,109,11,0.1);color:#111827;border:1px solid rgba(16,24,40,0.3);">Dibebaskan</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        @if($billing->status !== 'paid')
                                            <button wire:click="openPayment({{ $billing->id }})"
                                                ...
                                                style="background:#111827;color:#ffffff;"
                                                onmouseover="this.style.background='#1f2a37'" onmouseout="this.style.background='#1f2a37'">Bayar</button>
                                            <button wire:click="openWaive({{ $billing->id }})"
                                                class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold transition-colors"
                                                style="background:#f5f6f8;color:#111827;border:1px solid rgba(16,24,40,0.3);"
                                                onmouseover="this.style.background='#e4e7ec'" onmouseout="this.style.background='#f5f6f8'">Bebaskan</button>
                                        @elseif($billing->is_waived)
                                            <button wire:click="cancelWaive({{ $billing->id }})"
                                                ...
                                                style="background:#f5f6f8;color:#98a2b3;border:1px solid #e4e7ec;"
                                                onmouseover="this.style.color='#c0453b'" onmouseout="this.style.color='#98a2b3'">Batalkan</button>
                                        @else
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-4 py-14 text-center">
                                    <svg class="w-12 h-12 mx-auto mb-3 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#111827"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    <p class="font-medium text-sm" style="color:#98a2b3;">Belum ada tagihan untuk periode ini</p>
                                    <p class="text-xs mt-1" style="color:#98a2b3;">Klik "Generate Tagihan" untuk membuat tagihan otomatis.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                        ...
                        <div class="flex items-center justify-between gap-2">
                            <div class="flex items-center gap-2 min-w-0">
                                ...
                                @else
                                    <span style="color:#98a2b3;">—</span>
                                @endif
                                <span class="font-medium truncate" style="color:#1d2939;">{{ $billing->responsibleResident?->name ?? '—' }}</span>
                            </div>
                            <div class="flex items-center gap-1 shrink-0">
                                ...
                                @endif
                                @if($billing->is_waived)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" style="background:rgba(139,109,11,0.1);color:#111827;border:1px solid rgba(16,24,40,0.3);">Dibebaskan</span>
                                @endif
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3 text-sm">
                            <div>
                                <span class="block text-xs" style="color:#98a2b3;">Total</span>
                                <span style="color:#1d2939;">Rp {{ number_format($billing->total_amount, 0, ',', '.') }}</span>
                            </div>
                            <div>
                                <span class="block text-xs" style="color:#98a2b3;">Terbayar</span>
                                <span style="color:#12805c;">Rp {{ number_format($billing->total_paid, 0, ',', '.') }}</span>
                            </div>
                            <div>
                                <span class="block text-xs" style="color:#98a2b3;">Sisa</span>
                                <span style="{{ $billing->outstanding > 0 ? 'color:#c0453b;font-weight:600;' : 'color:#98a2b3;' }}">Rp {{ number_format($billing->outstanding, 0, ',', '.') }}</span>
                            </div>
                            <div>
                                <span class="block text-xs" style="color:#98a2b3;">Security / Sampah / Kas RT</span>
                                <span class="font-mono text-xs" style="color:#7c8698;">
                        ...
                        <div class="mt-3">
                            ...
                            @elseif($billing->is_waived)
                                <button wire:click="cancelWaive({{ $billing->id }})"
                                    ...
                                    style="background:#f5f6f8;color:#98a2b3;border:1px solid #e4e7ec;">Batalkan Pembebasan</button>
                            @else
            ...
            @if($billings->hasPages())
                <div class="px-4 py-3" style="border-top:1px solid #eef0f3;">{{ $billings->links() }}</div>
            @endif
        </div>
        @else
            <div class="rounded-2xl px-4 py-8 text-center" style="background:rgba(16,24,40,0.05);border:1px solid rgba(16,24,40,0.15);">
                <svg class="w-12 h-12 mx-auto mb-3 opacity-30" fill="none" viewBox="0 0 24 24" stroke="#111827"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                <p class="font-medium" style="color:#111827;">Pilih atau buat periode IPL terlebih dahulu</p>
            </div>
        @endif
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="background:#ffffff;border:1px solid #d0d5dd;max-height:90vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:#f2f4f7;border-bottom:1px solid rgba(16,24,40,0.35);">
                <h3 class="font-bold" style="color:#1d2939;font-family:'IBM Plex Sans',serif;">{{ $editingPeriodId ? 'Edit Periode IPL' : 'Buat Periode IPL Baru' }}</h3>
                <button wire:click="closePeriodModal()" class="p-1 rounded-lg transition-colors" style="color:#1d2939;"
            ...
            <form wire:submit="savePeriod" class="overflow-y-auto px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Tahun <span style="color:#c0453b;">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Bulan <span style="color:#c0453b;">*</span></label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Iuran Security (Rp) <span style="color:#c0453b;">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Biaya Sampah (Rp) <span style="color:#c0453b;">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Iuran Kas RT (Rp) <span style="color:#c0453b;">*</span></label>
                ...
                <div class="rounded-xl p-4 space-y-3" style="background:#ffffff;border:1px solid #e4e7ec;">
                    <p class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">Biaya Tambahan</p>
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($extraTariffRates as $typeId => $amount)
                            @php $tariffType = $extraTariffTypes->firstWhere('id', $typeId) @endphp
                            <div>
                                <label class="block text-xs font-medium mb-1" style="color:#475467;">{{ $tariffType?->name ?? 'Tarif #'.$typeId }} (Rp)</label>
                                <input type="number" wire:model="extraTariffRates.{{ $typeId }}" min="0" step="1000"
                                    style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);color:#1d2939;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;"
                                    onfocus="this.style.borderColor='#111827'" onblur="this.style.borderColor='#e4e7ec'">
                ...
                <div>
                    <label class="block text-sm font-medium mb-1" style="color:#475467;">Catatan</label>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="background:#ffffff;border:1px solid #d0d5dd;max-height:90vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:linear-gradient(135deg,#e3f1ea,#e3f1ea);border-bottom:1px solid rgba(18,128,92,0.35);">
                <h3 class="font-bold" style="color:#12805c;font-family:'IBM Plex Sans',serif;">Catat Pembayaran IPL</h3>
            ...
            <div class="overflow-y-auto">
                @if($payingBilling)
                    <div class="mx-6 mt-4 rounded-xl p-4 text-sm" style="background:#ffffff;border:1px solid #e4e7ec;">
                        <div class="font-semibold text-base" style="color:#1d2939;">{{ $payingBilling->responsibleResident?->name ?? '(Tanpa Penanggung Jawab)' }}</div>
                        <div class="text-xs mt-0.5" style="color:#7c8698;">Blok: <span class="font-mono font-bold" style="color:#111827;">{{ $payingBilling->houseBlock?->block_code ?? '—' }}</span></div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-3">
                            <div class="rounded-lg p-2.5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                                <div class="text-xs" style="color:#98a2b3;">Tagihan Security</div>
                                <div class="font-semibold text-sm mt-0.5" style="color:#1d2939;">Rp {{ number_format($payingBilling->ipl_security_amount, 0, ',', '.') }}</div>
                            </div>
                            <div class="rounded-lg p-2.5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                                <div class="text-xs" style="color:#98a2b3;">Tagihan Sampah</div>
                                <div class="font-semibold text-sm mt-0.5" style="color:#1d2939;">Rp {{ number_format($payingBilling->ipl_garbage_amount, 0, ',', '.') }}</div>
                            </div>
                            <div class="rounded-lg p-2.5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                                <div class="text-xs" style="color:#98a2b3;">Tagihan Kas RT</div>
                                <div class="font-semibold text-sm mt-0.5" style="color:#1d2939;">Rp {{ number_format($payingBilling->ipl_kas_rt_amount, 0, ',', '.') }}</div>
                            </div>
                ...
                <form wire:submit="savePayment" class="p-6 space-y-4">
                    ...
                    <div class="rounded-xl p-4 space-y-3" style="background:#ffffff;border:1px solid #e4e7ec;">
                        <div class="flex items-center justify-between">
                            <p class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">Jumlah Bulan</p>
                            @if($paymentMonths > 1)
                                <span class="text-xs px-2 py-0.5 rounded-full font-medium" style="background:rgba(16,24,40,0.15);color:#111827;">Multi-bulan aktif</span>
                            @endif
                        </div>
                        <div class="flex gap-2 flex-wrap">
                            @foreach([1, 3, 6, 12] as $n)
                                <button type="button" wire:click="$set('paymentMonths', {{ $n }})"
                                    ...
                                    style="{{ $paymentMonths == $n ? 'background:#111827;color:#ffffff;' : 'background:#eef0f3;color:#7c8698;border:1px solid #e4e7ec;' }}"
                                    @if($paymentMonths != $n) onmouseover="this.style.background='#e4e7ec';this.style.color='#111827'" onmouseout="this.style.background='#eef0f3';this.style.color='#7c8698'" @endif>
                                    {{ $n == 1 ? '1 bulan' : $n . ' bln' }}
                                </button>
                            @endforeach
                            <div class="flex items-center gap-1.5">
                                <input type="number" wire:model.live="paymentMonths" min="1" max="12"
                                    ...
                                    style="background:#eef0f3;border:1px solid #e4e7ec;color:#1d2939;border-radius:0.5rem;padding:0.375rem 0.5rem;outline:none;"
                                    onfocus="this.style.borderColor='#111827'" onblur="this.style.borderColor='#e4e7ec'">
                                <span class="text-xs" style="color:#98a2b3;">bln</span>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-2 pt-1">
                            <button type="button" wire:click="$set('paymentDirection', 'forward')"
                                ...
                                style="{{ $paymentDirection === 'forward' ? 'background:rgba(16,24,40,0.2);color:#111827;border:1px solid rgba(16,24,40,0.4);' : 'background:#ffffff;color:#98a2b3;border:1px solid #e4e7ec;' }}">
                                ➡ Ke Depan (Muka)
                            </button>
                            <button type="button" wire:click="$set('paymentDirection', 'backward')"
                                class="flex-1 py-2 rounded-lg text-xs font-medium transition-colors text-center"
                                style="{{ $paymentDirection === 'backward' ? 'background:rgba(192,69,59,0.15);color:#c0453b;border:1px solid rgba(192,69,59,0.35);' : 'background:#ffffff;color:#98a2b3;border:1px solid #e4e7ec;' }}">
                    ...
                    <div class="rounded-xl overflow-hidden" style="border:1px solid #e4e7ec;">
                        <div class="px-4 py-2" style="background:#ffffff;border-bottom:1px solid #f5f6f8;">
                            <span class="text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Rincian Bulan yang Dicakup</span>
                        </div>
                        <div>
                            @foreach($multiMonthPreview as $row)
                            <div class="flex items-center justify-between px-4 py-2" style="border-bottom:1px solid #eef0f3;">
                                <div class="flex items-center gap-2">
                                    @if($row['status'] === 'paid')
                                        <span class="w-1.5 h-1.5 rounded-full shrink-0" style="background:#12805c;"></span>
                                    @elseif($row['status'] === 'no_billing')
                                        <span class="w-1.5 h-1.5 rounded-full shrink-0" style="background:#cdd2da;"></span>
                                    @elseif($row['status'] === 'partial')
                                        <span class="w-1.5 h-1.5 rounded-full shrink-0" style="background:#c77d1a;"></span>
                                    @else
                                        <span class="w-1.5 h-1.5 rounded-full shrink-0" style="background:#111827;"></span>
                                    @endif
                                    <span class="text-sm" style="color:#1d2939;">{{ $row['label'] }}</span>
                                    @if(!$row['period_exists'] && $row['status'] !== 'no_billing')
                                        <span class="text-xs px-1 py-0.5 rounded" style="background:rgba(16,24,40,0.1);color:#111827;">baru</span>
                                    @endif
                                </div>
                                <div class="text-right text-xs">
                                    ...
                                    @elseif($row['status'] === 'no_billing')
                                        <span style="color:#98a2b3;">Tidak ada tagihan</span>
                                    @else
                                        <span class="font-mono font-semibold" style="color:#1d2939;">Rp {{ number_format($row['outstanding'], 0, ',', '.') }}</span>
                        ...
                        <div class="flex items-center justify-between px-4 py-3" style="background:#ffffff;">
                            <span class="text-sm font-semibold" style="color:#475467;">Total yang akan dibayar</span>
                            <span class="text-base font-bold font-mono" style="color:#111827;">Rp {{ number_format($multiMonthTotal, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    @endif

                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Tanggal Bayar <span style="color:#c0453b;">*</span></label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                            <label class="block text-sm font-medium mb-1" style="color:#475467;">Security (Rp) <span style="color:#c0453b;">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1" style="color:#475467;">Sampah (Rp) <span style="color:#c0453b;">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1" style="color:#475467;">Kas RT (Rp) <span style="color:#c0453b;">*</span></label>
                    ...
                    <div class="rounded-xl p-4 space-y-3" style="background:#ffffff;border:1px solid #e4e7ec;">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($extraChargePayments as $typeId => $amount)
                                ...
                                <div>
                                    <label class="block text-xs font-medium mb-1" style="color:#475467;">
                                        {{ $chargeItem?->tariffType?->name ?? 'Biaya #'.$typeId }} (Rp)
                                        @if($chargeItem)
                                            <span class="ml-1 text-xs" style="color:#98a2b3;">/ Rp {{ number_format($chargeItem->billed_amount, 0, ',', '.') }}</span>
                                        @endif
                                    </label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1" style="color:#475467;">Diterima Oleh</label>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit" wire:loading.attr="disabled"
                            ...
                            style="background:#12805c;color:#1d2939;"
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col" style="background:#ffffff;border:1px solid #d0d5dd;max-height:90vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:#f2f4f7;border-bottom:1px solid #344054;">
                <h3 class="font-bold text-white" style="font-family:'IBM Plex Sans',serif;">Bayar IPL per Penghuni</h3>
            ...
            <div class="overflow-y-auto">
                ...
                <form wire:submit="saveChecklistPayment" class="p-6 space-y-4">
                    ...
                    @if($checklistResidentId)
                        <div class="rounded-xl overflow-hidden" style="border:1px solid #e4e7ec;">
                            <div class="flex items-center gap-3 px-4 py-3" style="background:#ffffff;border-bottom:1px solid #f5f6f8;">
                                <input type="checkbox" wire:click="toggleChecklistAll"
                                    style="width:1rem;height:1rem;accent-color:#111827;"
                                    {{ count($checklistSelectedIds) > 0 && count($checklistSelectedIds) === $checklistBillings->count() ? 'checked' : '' }}>
                                <span class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">
                                    Pilih Semua
                                    <span class="ml-1 font-normal" style="color:#98a2b3;">({{ $checklistBillings->count() }} tagihan)</span>
                                </span>
                            </div>

                            @if($checklistBillings->isEmpty())
                                <div class="px-4 py-8 text-center">
                                    <p class="text-sm" style="color:#98a2b3;">Tidak ada tagihan yang perlu dibayar untuk penghuni ini.</p>
                                </div>
                            @else
                                <div class="divide-y" style="border-color:#eef0f3;">
                                    @foreach($checklistBillings as $billing)
                                        ...
                                        <label class="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors"
                                            ...
                                            onmouseover="this.style.backgroundColor='#f5f6f8'" onmouseout="this.style.backgroundColor='{{ in_array($billing->id, $checklistSelectedIds) ? 'rgba(18,128,92,0.04)' : '' }}'">
                                            ...
                                            <div class="flex-1 min-w-0 flex items-center gap-3">
                                                ...
                                                <span class="text-sm font-medium truncate" style="color:#1d2939;">{{ $billing->period?->period_label ?? '—' }}</span>
                                            </div>
                                            <div class="text-right shrink-0">
                                                <div class="text-sm font-semibold" style="color:#1d2939;">Rp {{ number_format($billing->total_amount, 0, ',', '.') }}</div>
                                                @if($outstanding > 0)
                    ...
                    @else
                        <div class="rounded-xl px-4 py-8 text-center" style="background:#ffffff;border:1px solid #e4e7ec;">
                            <p class="text-sm" style="color:#98a2b3;">Pilih penghuni terlebih dahulu untuk melihat tagihan yang perlu dibayar.</p>
                        </div>
                    @endif
                    ...
                    @if(count($checklistSelectedIds) > 0)
                        <div class="rounded-xl p-4 space-y-3" style="background:#ffffff;border:1px solid #e4e7ec;">
                            <p class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">Detail Pembayaran</p>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-sm font-medium mb-1" style="color:#475467;">Tanggal Bayar <span style="color:#c0453b;">*</span></label>
                                    <input type="date" wire:model="checkPayDate"
                                        style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);color:#1d2939;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;color-scheme:dark;"
                                        onfocus="this.style.borderColor='#111827'" onblur="this.style.borderColor='#e4e7ec'">
                                    @error('checkPayDate')<p class="text-xs mt-1" style="color:#c0453b;">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1" style="color:#475467;">Metode <span style="color:#c0453b;">*</span></label>
                            ...
                            <div>
                                ...
                                <textarea wire:model="checkPayNotes" rows="2"
                                    style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);color:#1d2939;border-radius:0.75rem;padding:0.5rem 0.75rem;width:100%;font-size:0.875rem;outline:none;resize:none;"
                                    onfocus="this.style.borderColor='#111827'" onblur="this.style.borderColor='#e4e7ec'"></textarea>
                            </div>
                        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="background:#ffffff;border:1px solid #d0d5dd;max-height:90vh;" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 shrink-0 rounded-t-2xl" style="background:linear-gradient(135deg,#eef0f3,#eef0f3);border-bottom:1px solid rgba(16,24,40,0.4);">
                <h3 class="font-bold" style="color:#111827;font-family:'IBM Plex Sans',serif;">Bebaskan Tunggakan (Pemutihan)</h3>
                <button wire:click="closeWaiveModal()" class="p-1 rounded-lg transition-colors" style="color:#111827;"
                    onmouseover="this.style.background='rgba(16,24,40,0.15)'" onmouseout="this.style.background=''">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <div class="overflow-y-auto">
                @if($waivingBilling)
                    <div class="mx-6 mt-4 rounded-xl p-4 text-sm" style="background:#ffffff;border:1px solid #e4e7ec;">
                        <div class="font-semibold text-base" style="color:#1d2939;">{{ $waivingBilling->responsibleResident?->name ?? '(Tanpa Penanggung Jawab)' }}</div>
                        <div class="text-xs mt-0.5" style="color:#7c8698;">
                            Blok: <span class="font-mono font-bold" style="color:#111827;">{{ $waivingBilling->houseBlock?->block_code ?? '—' }}</span>
                            &middot; Periode: <span class="font-medium" style="color:#1d2939;">{{ $waivingBilling->period?->period_label ?? '—' }}</span>
                        </div>
                    </div>
                @endif

                <div class="mx-6 mt-3 rounded-xl p-3 text-xs" style="background:rgba(16,24,40,0.08);border:1px solid rgba(16,24,40,0.3);color:#111827;">
                ...
                <form wire:submit="saveWaive" class="p-6 space-y-4">
                    @if($waivingBilling)
                    <div class="space-y-2">
                        <p class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">Komponen yang Dibebaskan</p>
                        @php
                            $waiveRows = [
                            ...
                            <label class="flex items-center justify-between gap-3 rounded-xl px-4 py-3 cursor-pointer {{ $row['rem'] <= 0 ? 'opacity-50' : '' }}"
                                ...
                                <span class="flex items-center gap-2.5">
                                    <input type="checkbox" wire:model="{{ $row['model'] }}" @disabled($row['rem'] <= 0)
                                        style="width:1rem;height:1rem;accent-color:#111827;">
                                    <span class="text-sm font-medium" style="color:#1d2939;">{{ $row['label'] }}</span>
                                </span>
                                <span class="text-sm font-mono font-semibold" style="color:{{ $row['rem'] > 0 ? '#c0453b' : '#98a2b3' }};">
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:#475467;">Alasan Pembebasan <span style="color:#c0453b;">*</span></label>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit" wire:loading.attr="disabled"
                            class="inline-flex items-center gap-2 px-5 py-2 text-sm rounded-xl font-semibold transition-colors disabled:opacity-50"
                            style="background:#111827;color:#ffffff;"
                            onmouseover="this.style.background='#475467'" onmouseout="this.style.background='#1f2a37'">
