<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-bold" style="color:#111827;font-family:'IBM Plex Sans',serif;">Anggota Keluarga</h2>
            <p class="text-sm mt-0.5" style="color:#667085;">Kelola data anggota keluarga yang tinggal bersama Anda</p>
        </div>
    ...
    @php
        ...
        $relColors = [
            ...
            'orang_tua'  => 'background:rgba(16,24,40,0.1);color:#111827;border:1px solid rgba(16,24,40,0.25);',
            ...
            'lainnya'    => 'background:rgba(107,114,128,0.1);color:#475467;border:1px solid rgba(107,114,128,0.25);',
        ];
    @endphp

    {{-- Member List --}}
    @if($members->isEmpty())
        <div class="rounded-2xl p-6 sm:p-12 text-center" style="background:#ffffff;border:1px dashed #e4e7ec;">
            ...
            <p class="text-sm font-medium" style="color:#98a2b3;">Belum ada anggota keluarga</p>
            <p class="text-xs mt-1" style="color:#98a2b3;">Klik "Tambah Anggota" untuk mulai menambahkan data keluarga</p>
        </div>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            @foreach($members as $member)
            <div class="rounded-2xl p-5 flex items-start gap-4" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);"
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <span class="font-semibold text-sm" style="color:#1d2939;">{{ $member->name }}</span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                            style="{{ $relColors[$member->relationship] ?? 'background:#f5f6f8;color:#667085;border:1px solid #e4e7ec;' }}">
                            {{ $relLabels[$member->relationship] ?? ucfirst($member->relationship) }}
                        </span>
                    </div>
                    <div class="flex items-center gap-3 mt-1.5 text-xs flex-wrap" style="color:#7c8698;">
                        <span style="color:{{ $member->gender === 'laki-laki' ? '#2563eb' : '#db2777' }};">
                            {{ $member->gender === 'laki-laki' ? 'Laki-laki' : 'Perempuan' }}
                        </span>
                        @if($member->birth_date)
                            <span style="color:#98a2b3;">&middot; {{ $member->birth_date->format('d M Y') }}</span>
                        @endif
                        @if($member->nik)
                            <span style="color:#98a2b3;">&middot; NIK: {{ $member->nik }}</span>
                        @endif
                    </div>
                    @if($member->notes)
                        <p class="text-xs mt-1.5 italic" style="color:#98a2b3;">{{ $member->notes }}</p>
                    @endif
                </div>
