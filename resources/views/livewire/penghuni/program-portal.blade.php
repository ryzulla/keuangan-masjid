<div>
    ...
    {{-- Header — Denah hero --}}
    <div class="pp-reveal relative overflow-hidden rounded-[22px] p-6 mb-6" style="background:linear-gradient(150deg,#0B2E28,#164A40 72%);color:#F4EFE2;">
        <div class="pp-denah"></div>
        <div class="relative">
            <p class="pp-eyebrow" style="color:rgba(244,239,226,0.6);">Gotong royong warga</p>
            <h3 class="pp-display" style="font-weight:500;font-size:24px;color:#F4EFE2;margin-top:3px;">Program & Kampanye Aktif</h3>
            <p class="text-sm mt-1.5" style="color:rgba(244,239,226,0.72);">Dukung program perumahan dan masjid dengan berdonasi</p>
        </div>
    </div>
    ...
    @php
        ...
        $orgStyle  = $campaign->organization_type === 'dkm'
            ...
            : 'background:rgba(22,74,64,0.12);color:#17231E;border:1px solid rgba(22,74,64,0.25);';
    ...
    @endif
        ...
        <div class="rounded-2xl overflow-hidden flex flex-col" style="background:#ffffff;border:1px solid #E0DFD4;box-shadow:0 1px 2px rgba(22,74,64,0.04),0 8px 20px -8px rgba(22,74,64,0.06);">
            ...
            <a href="{{ route('penghuni.program.detail', $campaign->id) }}" wire:navigate
               class="block w-full h-40 shrink-0 overflow-hidden relative" style="background:#ffffff;">
                ...
                @else
                    <div class="w-full h-full flex items-center justify-center text-4xl font-bold" style="color:rgba(22,74,64,0.2);">
            ...
            <div class="p-4 flex flex-col flex-1">
                <div class="flex items-start justify-between gap-2 mb-2">
                    <a href="{{ route('penghuni.program.detail', $campaign->id) }}" wire:navigate
                       class="font-semibold text-sm leading-snug hover:underline" style="color:#17231E;">{{ $campaign->name }}</a>
                    <span class="shrink-0 text-xs px-2 py-0.5 rounded-full font-medium" style="{{ $orgStyle }}">{{ $orgLabel }}</span>
                </div>

                @if($campaign->description)
                    <p class="text-xs mb-3 line-clamp-2" style="color:#909A8F;">{{ strip_tags($campaign->description) }}</p>
                @endif
                ...
                <div class="mb-3 mt-auto">
                    ...
                    <div class="w-full rounded-full h-1.5" style="background:#E0DFD4;">
                        <div class="h-1.5 rounded-full" style="width:{{ $percent }}%;background:linear-gradient(90deg,#164A40,#164A40);"></div>
                    </div>
                    <div class="flex justify-between text-xs mt-1">
                        <span style="color:#17231E;">Rp {{ number_format($collected, 0, ',', '.') }}</span>
                        <span style="color:#909A8F;">dari Rp {{ number_format($campaign->target_amount, 0, ',', '.') }}</span>
                    </div>
                </div>
    ...
    @endif
    @empty
        <div class="rounded-2xl p-6 sm:p-12 text-center" style="background:#ffffff;border:1px solid #E0DFD4;box-shadow:0 1px 2px rgba(22,74,64,0.04),0 8px 20px -8px rgba(22,74,64,0.06);">
            <svg class="w-12 h-12 mx-auto mb-3 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#164A40"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
            <p class="font-medium text-sm" style="color:#909A8F;">Belum ada program aktif saat ini</p>
        </div>
    @endforelse
