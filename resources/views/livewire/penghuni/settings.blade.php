<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style="background:rgba(21,99,223,0.12);border:1px solid rgba(21,99,223,0.35);">
            <svg class="w-4.5 h-4.5" style="width:1.15rem;height:1.15rem;" fill="none" viewBox="0 0 24 24" stroke="#1563df" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
        </div>
        <div>
            <h1 class="text-lg font-bold" style="color:#161e2d;font-family:'Manrope',serif;">Pengaturan</h1>
            <p class="text-xs" style="color:#a3abb0;">Kelola profil, keamanan, dan preferensi akun Anda</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
        ...
        {{-- ══════════ PROFIL ══════════ --}}
        <section class="rounded-2xl p-5 sm:p-6" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
            <h2 class="text-sm font-semibold mb-1" style="color:#161e2d;">Profil</h2>
            <p class="text-xs mb-4" style="color:#a3abb0;">Data kontak Anda</p>
            ...
            <form wire:submit="saveProfile" class="space-y-4">
                {{-- Nama --}}
                <div>
                    <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Nama <span style="color:#c0453b;">*</span></label>
                    <input wire:model="name" type="text" placeholder="Nama lengkap"
                        style="background:#ffffff;border:1px solid #e4e4e4;color:#161e2d;border-radius:0.75rem;padding:0.65rem 0.875rem;width:100%;font-size:0.875rem;outline:none;"
                ...
                {{-- Email --}}
                <div>
                    <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Email</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Nomor Telepon</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Nomor WhatsApp</label>
        ...
        {{-- ══════════ GANTI PASSWORD ══════════ --}}
        <section class="rounded-2xl p-5 sm:p-6" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
            <h2 class="text-sm font-semibold mb-1" style="color:#161e2d;">Ganti Password</h2>
            <p class="text-xs mb-4" style="color:#a3abb0;">Gunakan password yang kuat dan mudah Anda ingat</p>
            ...
            <form wire:submit="savePassword" class="space-y-4">
                <div>
                    <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Password Saat Ini <span style="color:#c0453b;">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Password Baru <span style="color:#c0453b;">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium mb-1.5" style="color:#5c6368;">Konfirmasi Password Baru <span style="color:#c0453b;">*</span></label>
        ...
        {{-- ══════════ FOTO PROFIL ══════════ --}}
        <section class="rounded-2xl p-5 sm:p-6" style="background:#ffffff;border:1px solid #f7f7f7;">
            <h2 class="text-sm font-semibold mb-1" style="color:#161e2d;">Foto Profil</h2>
            <p class="text-xs mb-4" style="color:#a3abb0;">Format gambar, maksimal 2MB</p>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <div class="flex justify-center sm:block">
                    @if($photo)
                        <img src="{{ $photo->temporaryUrl() }}" class="w-24 h-24 rounded-2xl object-cover" style="border:1px solid rgba(21,99,223,0.4);">
                    @elseif($existingPhoto)
                        <img src="{{ Storage::disk('public')->url($existingPhoto) }}" class="w-24 h-24 rounded-2xl object-cover" style="border:1px solid rgba(21,99,223,0.4);">
                    @else
                        <div class="w-24 h-24 rounded-2xl flex items-center justify-center text-3xl font-bold" style="background:rgba(21,99,223,0.15);color:#161e2d;">
        ...
        <section class="rounded-2xl p-5 sm:p-6" style="background:#ffffff;border:1px solid #f7f7f7;">
            <h2 class="text-sm font-semibold mb-1" style="color:#161e2d;">Preferensi Notifikasi</h2>
            <p class="text-xs mb-4" style="color:#a3abb0;">Pilih jenis pemberitahuan yang ingin Anda terima</p>
            ...
            <form wire:submit="saveNotifications" class="space-y-3">
                @foreach($notificationTypes as $key => $label)
                    <label class="flex items-center justify-between gap-3 rounded-xl px-4 py-3.5 cursor-pointer transition-colors"
                        ...
                        onmouseover="this.style.borderColor='#d9d9d9'" onmouseout="this.style.borderColor='#e4e4e4'">
                        <span class="text-sm" style="color:#161e2d;">{{ $label }}</span>
