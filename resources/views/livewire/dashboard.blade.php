<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base" style="color:#161e2d;">Dashboard</h2>
    </x-slot>
    ...
    <div class="py-6">
        ...
        <div class="relative overflow-hidden rounded-2xl p-6 text-white"
            style="background:linear-gradient(135deg,#ffffff 0%,#ffffff 62%);border:1px solid rgba(21,99,223,0.35);box-shadow:0 4px 24px rgba(0,0,0,0.1);">
            ...
            {{-- Gold line accent --}}
            <div class="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl" style="background:linear-gradient(to bottom,#1563df,#1563df);"></div>
            <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pl-4">
                <div>
                    <p class="text-xs font-medium" style="color:#161e2d;">Selamat datang,</p>
                    <h1 class="text-2xl font-bold mt-0.5" style="color:#161e2d;font-family:'Manrope',serif;">{{ auth()->user()->name }}</h1>
                    <p class="text-sm mt-1" style="color:#a3abb0;">
                        {{ \Carbon\Carbon::now()->locale('id')->isoFormat('dddd, D MMMM Y') }}
                    </p>
                </div>
                <div class="text-right hidden sm:block">
                    <div class="text-4xl font-light" style="color:#161e2d;opacity:0.8;">{{ \Carbon\Carbon::now()->format('H:i') }}</div>
                    <div class="text-xs mt-1" style="color:#a3abb0;">Sistem Manajemen Perumahan &amp; DKM</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {{-- Kas Perumahan --}}
            <div class="relative overflow-hidden rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                <div class="absolute top-0 right-0 w-16 h-16 -mt-3 -mr-3 rounded-full opacity-20" style="background:#1563df;filter:blur(20px);"></div>
                <div class="relative">
                    <div class="flex items-center justify-between mb-3">
                        <div class="w-9 h-9 flex items-center justify-center rounded-xl" style="background:rgba(21,99,223,0.12);border:1px solid rgba(21,99,223,0.2);">
                            <svg class="w-4 h-4" style="color:#161e2d;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        </div>
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full" style="background:rgba(21,99,223,0.12);color:#161e2d;border:1px solid rgba(21,99,223,0.2);">Perumahan</span>
                    </div>
                    <p class="text-xs font-medium" style="color:#a3abb0;">Total Kas Perumahan</p>
                    <p class="text-xl font-bold mt-0.5" style="color:#161e2d;">Rp {{ number_format($perumahanBalance ?? 0, 0, ',', '.') }}</p>
                    <p class="text-xs mt-1" style="color:#a3abb0;">{{ $totalAssignedBlocks ?? 0 }} unit terisi</p>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                ...
                <div class="relative">
                    ...
                    <p class="text-xs font-medium" style="color:#a3abb0;">IPL Terkumpul</p>
                    <p class="text-xl font-bold mt-0.5" style="color:#161e2d;">Rp {{ number_format($iplSummary['total_terbayar'] ?? 0, 0, ',', '.') }}</p>
                    <p class="text-xs mt-1" style="color:#a3abb0;">{{ $iplSummary['period'] ?? 'N/A' }}</p>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                ...
                <div class="relative">
                    ...
                    <p class="text-xs font-medium" style="color:#a3abb0;">Tunggakan IPL</p>
                    <p class="text-xl font-bold mt-0.5" style="color:#161e2d;">Rp {{ number_format($iplSummary['tunggakan'] ?? 0, 0, ',', '.') }}</p>
            ...
            {{-- Kas DKM --}}
            <div class="relative overflow-hidden rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                <div class="absolute top-0 right-0 w-16 h-16 -mt-3 -mr-3 rounded-full opacity-10" style="background:#1563df;filter:blur(20px);"></div>
                <div class="relative">
                    <div class="flex items-center justify-between mb-3">
                        <div class="w-9 h-9 flex items-center justify-center rounded-xl" style="background:rgba(21,99,223,0.1);border:1px solid rgba(21,99,223,0.2);">
                        ...
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full" style="background:rgba(21,99,223,0.1);color:#161e2d;border:1px solid rgba(21,99,223,0.2);">DKM</span>
                    </div>
                    <p class="text-xs font-medium" style="color:#a3abb0;">Total Kas DKM</p>
                    <p class="text-xl font-bold mt-0.5" style="color:#161e2d;">Rp {{ number_format($dkmBalance ?? 0, 0, ',', '.') }}</p>
                    <p class="text-xs mt-1" style="color:#a3abb0;">{{ $totalResidents ?? 0 }} penghuni terdaftar</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="rounded-2xl p-6" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h3 class="font-semibold text-sm" style="color:#161e2d;">Status IPL</h3>
                        <p class="text-xs mt-0.5" style="color:#a3abb0;">{{ $iplSummary['period'] ?? 'Belum ada periode' }}</p>
                    </div>
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg" style="background:rgba(21,99,223,0.1);border:1px solid rgba(21,99,223,0.2);">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6">
                    <div class="relative w-40 h-40 flex-shrink-0">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span class="text-2xl font-bold" style="color:#161e2d;">{{ $iplSummary['jumlah_unit'] ?? 0 }}</span>
                            <span class="text-xs" style="color:#a3abb0;">Total Unit</span>
                        </div>
                    </div>
                    <div class="flex flex-col gap-2.5 w-full">
                        <div class="flex items-center justify-between p-3 rounded-xl" style="background:rgba(18,128,92,0.08);border:1px solid rgba(18,128,92,0.15);">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm" style="color:#5c6368;">Lunas</span>
                            </div>
                            <div class="text-right">
                                <div class="font-bold" style="color:#12805c;">{{ $iplSummary['jumlah_lunas'] ?? 0 }}</div>
                                <div class="text-xs" style="color:#a3abb0;">unit</div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between p-3 rounded-xl" style="background:rgba(199,125,26,0.08);border:1px solid rgba(199,125,26,0.15);">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm" style="color:#5c6368;">Kurang Bayar</span>
                            </div>
                            <div class="text-right">
                        ...
                        <div class="flex items-center justify-between p-3 rounded-xl" style="background:rgba(192,69,59,0.08);border:1px solid rgba(192,69,59,0.15);">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm" style="color:#5c6368;">Belum Bayar</span>
                            </div>
                            <div class="text-right">
                        ...
                        <div class="mt-1">
                            <div class="flex justify-between text-xs mb-1.5">
                                <span style="color:#a3abb0;">Progres Pembayaran</span>
                                @php
                ...
                <div class="mt-4 pt-4 grid grid-cols-2 gap-3" style="border-top:1px solid #e4e4e4;">
                    <div class="text-center">
                        <p class="text-xs" style="color:#a3abb0;">Total Tagihan</p>
                        <p class="font-semibold text-sm" style="color:#161e2d;">Rp {{ number_format($iplSummary['total_tagihan'] ?? 0, 0, ',', '.') }}</p>
                    </div>
                    <div class="text-center">
                        <p class="text-xs" style="color:#a3abb0;">Tunggakan</p>
                        <p class="font-semibold text-sm" style="color:#c0453b;">Rp {{ number_format($iplSummary['tunggakan'] ?? 0, 0, ',', '.') }}</p>
                    </div>
                </div>
                @else
                <div class="text-center py-10" style="color:#a3abb0;">
                    ...
                    <a href="{{ route('ipl.index') }}" wire:navigate class="text-xs hover:underline mt-1 inline-block" style="color:#161e2d;">Buat periode →</a>
                </div>
                @endif
            </div>
            ...
            <div class="rounded-2xl p-6" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h3 class="font-semibold text-sm" style="color:#161e2d;">Keuangan DKM</h3>
                        <p class="text-xs mt-0.5" style="color:#a3abb0;">{{ \Carbon\Carbon::now()->locale('id')->isoFormat('MMMM Y') }}</p>
                    </div>
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg" style="background:rgba(21,99,223,0.1);border:1px solid rgba(21,99,223,0.2);">
                ...
                <div class="mt-4 grid grid-cols-2 gap-3">
                    <div class="rounded-xl p-3" style="background:rgba(18,128,92,0.08);border:1px solid rgba(18,128,92,0.15);">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs" style="color:#a3abb0;">Pemasukan</span>
                        </div>
                        <p class="font-bold text-base" style="color:#12805c;">Rp {{ number_format($monthlyIncomeDkm ?? 0, 0, ',', '.') }}</p>
                        <p class="text-xs" style="color:#a3abb0;">bulan ini</p>
                    </div>
                    <div class="rounded-xl p-3" style="background:rgba(192,69,59,0.08);border:1px solid rgba(192,69,59,0.15);">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs" style="color:#a3abb0;">Pengeluaran</span>
                        </div>
                        <p class="font-bold text-base" style="color:#c0453b;">Rp {{ number_format($monthlyExpenseDkm ?? 0, 0, ',', '.') }}</p>
                        <p class="text-xs" style="color:#a3abb0;">bulan ini</p>
                    </div>
                </div>
                <div class="mt-3 p-3 rounded-xl" style="background:rgba(21,99,223,0.06);border:1px solid rgba(21,99,223,0.15);">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium" style="color:#161e2d;">Selisih Bersih</span>
                        @php $netDkm = ($monthlyIncomeDkm ?? 0) - ($monthlyExpenseDkm ?? 0); @endphp
                        <span class="font-bold {{ $netDkm >= 0 ? '' : '' }}" style="{{ $netDkm >= 0 ? 'color:#161e2d;' : 'color:#c0453b;' }}">
        ...
        <div class="rounded-2xl p-6" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
            <div class="flex items-center justify-between mb-5">
                <h3 class="font-semibold text-sm flex items-center gap-2" style="color:#161e2d;">
                    <svg class="w-4 h-4" style="color:#161e2d;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                    Program &amp; Kampanye Aktif
                </h3>
                <a href="{{ route('campaigns.index') }}" wire:navigate class="text-xs font-medium hover:underline" style="color:#161e2d;">Lihat semua →</a>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                @endphp
                <div class="rounded-xl p-4 transition-all hover:border-opacity-50" style="background:#ffffff;border:1px solid #e4e4e4;">
                    <div class="flex items-start justify-between mb-3">
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full"
                            style="{{ $isDkm ? 'background:rgba(18,128,92,0.1);color:#12805c;border:1px solid rgba(18,128,92,0.2);' : 'background:rgba(21,99,223,0.1);color:#161e2d;border:1px solid rgba(21,99,223,0.2);' }}">
                            {{ $isDkm ? 'DKM' : 'RT' }}
                        </span>
                        <span class="text-xs font-bold" style="{{ $isDkm ? 'color:#12805c;' : 'color:#161e2d;' }}">{{ $pct }}%</span>
                    </div>
                    <h4 class="font-semibold text-sm leading-tight mb-3 line-clamp-2" style="color:#161e2d;">{{ $c->name }}</h4>
                    <div class="w-full rounded-full h-1 mb-2" style="background:#e4e4e4;">
                        <div class="h-1 rounded-full transition-all" style="width:{{ $pct }}%;background:{{ $isDkm ? '#12805c' : '#1563df' }};"></div>
                    </div>
                    <div class="flex justify-between text-xs" style="color:#a3abb0;">
        ...
        <div class="rounded-2xl overflow-hidden" style="background:#ffffff;border:1px solid #e4e4e4;box-shadow:0 1px 2px rgba(21,99,223,0.04),0 8px 20px -8px rgba(21,99,223,0.06);">
            <div class="px-6 py-4 flex items-center justify-between" style="border-bottom:1px solid #f7f7f7;">
                <div>
                    <h3 class="font-semibold text-sm" style="color:#161e2d;">Rumah Disewakan</h3>
                    <p class="text-xs mt-0.5" style="color:#a3abb0;">{{ $contractedHouses->count() }} unit sedang disewa</p>
                </div>
                <a href="{{ route('residents.index') }}" wire:navigate class="text-xs font-medium hover:underline" style="color:#161e2d;">Lihat semua →</a>
            </div>
            <div class="overflow-x-auto hidden md:block">
                <table class="w-full text-sm">
                    <thead>
                        <tr style="border-bottom:1px solid #f7f7f7;">
                            <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Blok</th>
                            <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Penyewa</th>
                            <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Pemilik</th>
                            <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Sewa/Bulan</th>
                            <th class="text-left px-6 py-3 text-xs font-semibold uppercase tracking-wider" style="color:#a3abb0;">Kontrak Berakhir</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid #ffffff;" onmouseover="this.style.backgroundColor='#f7f7f7'" onmouseout="this.style.backgroundColor=''">
                            ...
                            <td class="px-6 py-3.5 text-xs" style="color:#5c6368;">
                                @php
                                    $owner = $ch->houseBlock?->owners?->first();
                                @endphp
                                {{ $owner->name ?? '—' }}
                            </td>
                            <td class="px-6 py-3.5 text-xs font-medium" style="color:#161e2d;">
                                @if($ch->monthly_rent)
                            ...
                            <td class="px-6 py-3.5 text-xs" style="color:{{ $endingSoon ? '#c77d1a' : '#5c6368' }};">
                ...
                donutEl._chartInstance = new Chart(donutEl, {
                    ...
                    options: {
                        ...
                        plugins: { legend: { display: false }, tooltip: {
                            backgroundColor: '#ffffff', borderColor: '#e4e4e4', borderWidth: 1,
                            titleColor: '#1563df', bodyColor: '#5c6368',
            ...
            if (barEl) {
                ...
                barEl._chartInstance = new Chart(barEl, {
                    ...
                    options: {
                        ...
                        scales: {
                            x: { grid: { color: '#f7f7f7' }, ticks: { color: '#a3abb0', callback: (v) => 'Rp ' + new Intl.NumberFormat('id-ID', {notation:'compact'}).format(v), font: { size: 10 } } },
                            y: { grid: { display: false }, ticks: { color: '#5c6368', font: { size: 12 } } }
                        }
                    }
                });
            }
