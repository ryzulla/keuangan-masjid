<div>
    <x-slot name="header">
        <h2 class="font-semibold text-base" style="color:#111827;">Detail Penghuni</h2>
    </x-slot>
        ...
        <div class="rounded-2xl p-6 mb-6" style="background:linear-gradient(135deg,#ffffff 0%,#ffffff 62%);border:1px solid rgba(16,24,40,0.35);">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    @endif
                    <div>
                        <h3 class="font-bold text-xl" style="color:#111827;font-family:'IBM Plex Sans',serif;">{{ $resident->name }}</h3>
                        <div class="flex items-center gap-2 mt-1">
                            ...
                            @else
                                <span class="text-xs px-2 py-0.5 rounded-full" style="background:#eef0f3;color:#7c8698;border:1px solid #e4e7ec;">Nonaktif</span>
                            @endif
                            <span class="text-xs" style="color:#7c8698;">{{ $resident->currentAssignments->count() }} unit aktif</span>
                            <span class="text-xs" style="color:#98a2b3;">&middot; {{ $resident->familyMembers->count() }} anggota keluarga</span>
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-5">
                ...
                <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                    <h4 class="text-xs font-semibold uppercase tracking-wider mb-4" style="color:#111827;">Informasi Kontak</h4>
                    <div class="space-y-3 text-sm">
                        <div>
                            <p class="text-xs" style="color:#98a2b3;">Telepon</p>
                            <p class="font-medium mt-0.5" style="color:#1d2939;">{{ $resident->phone ?: '—' }}</p>
                        </div>
                        <div style="border-top:1px solid #eef0f3;padding-top:0.75rem;">
                            <p class="text-xs" style="color:#98a2b3;">WhatsApp</p>
                            ...
                            @else
                                <p class="font-medium mt-0.5" style="color:#1d2939;">—</p>
                            @endif
                        </div>
                        @if($resident->email)
                        <div style="border-top:1px solid #eef0f3;padding-top:0.75rem;">
                            <p class="text-xs" style="color:#98a2b3;">Email</p>
                            <p class="font-medium mt-0.5 break-all" style="color:#1d2939;">{{ $resident->email }}</p>
                        </div>
                        @endif
                        @if($resident->notes)
                        <div style="border-top:1px solid #eef0f3;padding-top:0.75rem;">
                            <p class="text-xs" style="color:#98a2b3;">Catatan</p>
                            <p class="text-sm mt-0.5" style="color:#475467;">{{ $resident->notes }}</p>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                    <h4 class="text-xs font-semibold uppercase tracking-wider mb-4" style="color:#111827;">Kepemilikan & Hunian Saat Ini</h4>

                    @forelse($resident->currentAssignments as $a)
                    <div class="rounded-xl mb-3 overflow-hidden" style="border:1px solid #e4e7ec;" wire:key="ca-{{ $a->id }}">
                        <div class="flex items-center justify-between px-4 py-3" style="background:#ffffff;border-bottom:1px solid #eef0f3;">
                            <div class="flex items-center gap-3">
                                <span class="font-bold text-xl" style="color:#111827;">{{ $a->houseBlock?->block_code ?? '?' }}</span>
                                @if($a->is_primary_residence)
                                    <span class="text-xs px-2 py-0.5 rounded-full" style="background:rgba(16,24,40,0.1);color:#111827;border:1px solid rgba(16,24,40,0.2);">★ Domisili Utama</span>
                                @endif
                            </div>
                            <div class="flex gap-2">
                                ...
                                @if($a->houseBlock)
                                    <a href="{{ route('house-blocks.show', $a->houseBlock) }}" wire:navigate
                                        ...
                                        style="background:rgba(16,24,40,0.08);color:#111827;border:1px solid rgba(16,24,40,0.15);"
                        ...
                        <div class="px-4 py-2.5 text-xs flex flex-wrap gap-x-5 gap-y-1" style="color:#7c8698;">
                            @if($a->resident_since)
                    ...
                    @empty
                    <div class="text-center py-8 rounded-xl" style="border:1px dashed #e4e7ec;">
                        <p class="text-sm" style="color:#98a2b3;">Belum ada kepemilikan atau hunian aktif.</p>
                        <a href="{{ route('residents.edit', $resident) }}" wire:navigate class="text-xs mt-1 inline-block hover:underline" style="color:#111827;">Tetapkan rumah →</a>
                    </div>
                    @endforelse
                    ...
                    <div class="mt-4 pt-4" style="border-top:1px solid #e4e7ec;">
                        <h5 class="text-xs font-semibold uppercase tracking-wider mb-3" style="color:#98a2b3;">Riwayat Hunian Sebelumnya</h5>
                        <div class="space-y-2">
                            @foreach($historicalAssignments as $ha)
                            <div class="flex items-center justify-between py-2 px-3 rounded-lg text-xs" style="background:#ffffff;border:1px solid #eef0f3;">
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold" style="color:#7c8698;">{{ $ha->houseBlock?->block_code ?? '?' }}</span>
                                    <span style="color:#98a2b3;">{{ $ha->ownership_label }}</span>
                                </div>
                ...
                <div class="rounded-2xl p-5" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                    <div class="flex items-center justify-between mb-4">
                        <h4 class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">
                            Anggota Keluarga
                            @if($resident->familyMembers->count() > 0)
                                <span class="ml-2 text-xs font-normal" style="color:#7c8698;">{{ $resident->familyMembers->count() }} jiwa</span>
                            @endif
                        </h4>
                        <a href="{{ route('residents.edit', $resident) }}" wire:navigate
                            class="text-xs transition-colors" style="color:#111827;"
                            onmouseover="this.style.color='#111827'" onmouseout="this.style.color='#111827'">
                            Edit →
                        </a>
                    </div>
                    ...
                    <div class="flex items-center gap-3 py-2.5" style="border-bottom:1px solid #eef0f3;" wire:key="fm-{{ $fm->id }}">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium truncate" style="color:#1d2939;">{{ $fm->name }}</p>
                            <p class="text-xs" style="color:#98a2b3;">
                    ...
                    @empty
                    <p class="text-sm text-center py-6 italic" style="color:#98a2b3;">Belum ada anggota keluarga terdaftar.</p>
                    @endforelse
                </div>
            ...
            @endphp
            <div class="lg:col-span-3 rounded-2xl overflow-hidden mb-6" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
                <div class="px-5 py-4" style="border-bottom:1px solid #eef0f3;">
                    <h4 class="text-xs font-semibold uppercase tracking-wider" style="color:#111827;">Riwayat IPL</h4>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr style="background:#ffffff;border-bottom:1px solid #f5f6f8;">
                                <th class="text-left px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Periode</th>
                                <th class="text-left px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Blok</th>
                                <th class="text-right px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Total</th>
                                <th class="text-right px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Terbayar</th>
                                <th class="text-right px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Sisa</th>
                                <th class="text-center px-5 py-2.5 text-xs font-semibold uppercase tracking-wider" style="color:#98a2b3;">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($iplHistory as $billing)
                                ...
                                <tr style="border-bottom:1px solid #eef0f3;">
                                    <td class="px-5 py-2.5" style="color:#1d2939;">{{ $billing->period?->period_label ?? '?' }}</td>
                                    <td class="px-5 py-2.5" style="color:#667085;">{{ $billing->houseBlock?->block_code ?? '—' }}</td>
                                    <td class="px-5 py-2.5 text-right font-mono" style="color:#1d2939;">Rp {{ number_format($billing->total_amount, 0, ',', '.') }}</td>
                                    <td class="px-5 py-2.5 text-right font-mono" style="color:#12805c;">Rp {{ number_format($billing->total_paid, 0, ',', '.') }}</td>
                                    <td class="px-5 py-2.5 text-right font-mono" style="{{ $billing->outstanding > 0 ? 'color:#c0453b;' : 'color:#98a2b3;' }}">Rp {{ number_format($billing->outstanding, 0, ',', '.') }}</td>
                    ...
                    @foreach($iplHistory as $billing)
                        ...
                        <div class="px-5 py-3">
                            <div class="flex items-center justify-between gap-2">
                                <div class="min-w-0">
                                    <span class="font-medium" style="color:#1d2939;">{{ $billing->period?->period_label ?? '?' }}</span>
                                    @if($billing->houseBlock)<span class="ml-1 text-xs" style="color:#98a2b3;">{{ $billing->houseBlock->block_code }}</span>@endif
                                </div>
                            ...
                            <div class="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs">
                                <div><span class="block" style="color:#98a2b3;">Total</span><span class="font-mono" style="color:#1d2939;">Rp {{ number_format($billing->total_amount, 0, ',', '.') }}</span></div>
                                <div><span class="block" style="color:#98a2b3;">Terbayar</span><span class="font-mono" style="color:#12805c;">Rp {{ number_format($billing->total_paid, 0, ',', '.') }}</span></div>
                                <div><span class="block" style="color:#98a2b3;">Sisa</span><span class="font-mono" style="{{ $billing->outstanding > 0 ? 'color:#c0453b;' : 'color:#98a2b3;' }}">Rp {{ number_format($billing->outstanding, 0, ',', '.') }}</span></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                @if($resident->iplBillings->count() > 12)
                    <div class="px-5 py-2.5 text-center text-xs" style="color:#98a2b3;border-top:1px solid #eef0f3;">+{{ $resident->iplBillings->count() - 12 }} tagihan lainnya</div>
                @endif
            </div>
            @endif

        {{-- Portal Status Card (selalu tampil untuk semua penghuni) --}}
        <div class="rounded-2xl p-5 mt-6" style="background:#ffffff;border:1px solid #e4e7ec;box-shadow:0 1px 2px rgba(16,24,40,0.04),0 8px 20px -8px rgba(16,24,40,0.06);">
            <h4 class="text-xs font-semibold uppercase tracking-wider mb-3" style="color:#111827;">Akses Portal Penghuni</h4>
            @if($resident->email)
            <div class="flex items-start justify-between gap-3 flex-wrap">
                <div>
                    <p class="text-sm" style="color:#1d2939;">Login: <span style="color:#667085;">{{ $resident->email }}</span></p>
                    <p class="text-xs mt-1" style="color:#98a2b3;">
                        @if($resident->password)
                            <span style="color:#12805c;">Aktif</span> — Penghuni dapat login ke portal
                        @else
                            <span style="color:#7c8698;">Belum diatur</span> — Penghuni belum memiliki akses login
                        @endif
                    </p>
                </div>
                <div class="flex items-center gap-2">
            ...
            <div class="flex items-center justify-between gap-3 flex-wrap">
                <p class="text-sm" style="color:#475467;">Penghuni ini <span style="color:#c77d1a;">belum memiliki email</span>, sehingga belum bisa dibuatkan akses login portal. Tambahkan email penghuni terlebih dahulu.</p>
                <a href="{{ route('residents.edit', $resident) }}" wire:navigate
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative rounded-2xl shadow-2xl w-full max-w-sm" style="background:#ffffff;border:1px solid #d0d5dd;">
        <div class="px-6 py-4 rounded-t-2xl" style="background:#f2f4f7;border-bottom:1px solid rgba(16,24,40,0.35);">
            <h3 class="font-bold" style="color:#1d2939;font-family:'IBM Plex Sans',serif;">Atur Password Portal</h3>
            <p class="text-xs mt-1" style="color:#1d2939;">{{ $resident->name }} — {{ $resident->email }}</p>
        </div>
        <div class="px-6 py-5 space-y-4">
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:#475467;">Password Baru <span style="color:#c0453b;">*</span></label>
            ...
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:#475467;">Konfirmasi Password <span style="color:#c0453b;">*</span></label>
