<div x-data="{ open: false }" class="relative" @click.outside="open = false" @keydown.escape.window="open = false">
    {{-- Bell button --}}
    <button @click="open = !open" type="button" class="relative p-2 rounded-lg transition-colors" style="color:#5c6368;"
        onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#5c6368'" aria-label="Notifikasi">
        ...
        @if($this->badgeCount > 0)
            <span class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 px-0.5 items-center justify-center rounded-full text-xs font-bold"
                style="background:#c77d1a;color:#161e2d;font-size:9px;">{{ $this->badgeCount > 9 ? '9+' : $this->badgeCount }}</span>
        @endif
    </button>
    ...
    <div x-show="open" x-cloak
        ...
        {{-- Header --}}
        <div class="flex items-center justify-between px-4 py-3 shrink-0" style="border-bottom:1px solid #f7f7f7;">
            <span class="text-base sm:text-sm font-semibold" style="color:#161e2d;">Notifikasi</span>
            <button type="button" @click="open = false" class="p-1 -mr-1 rounded-lg transition-colors" style="color:#a3abb0;"
                onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#a3abb0'" aria-label="Tutup">
        ...
        <div class="flex items-center gap-4 px-4 py-2.5 shrink-0" style="border-bottom:1px solid #ffffff;">
            <button wire:click="markAllRead" wire:loading.attr="disabled"
                ...
                style="color:#161e2d;" onmouseover="this.style.color='#1563df'" onmouseout="this.style.color='#1563df'">
            ...
            <button wire:click="clearAll" wire:confirm="Hapus semua notifikasi? Tindakan ini tidak bisa dibatalkan."
                ...
                style="color:#a3abb0;" onmouseover="this.style.color='#c0453b'" onmouseout="this.style.color='#a3abb0'">
        ...
        @if($this->pendingPayCount > 0)
            <a href="/payment-requests" wire:navigate @click="open=false"
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs" style="color:#a3abb0;">Pembayaran penghuni perlu diverifikasi</p>
                </div>
            </a>
        @endif
        ...
        <div class="flex-1 overflow-y-auto overscroll-contain">
            @forelse($this->notifications as $notif)
                ...
                <button type="button" wire:key="notif-{{ $notif->id }}"
                    ...
                    style="{{ $notif->read_at ? '' : 'border-color:#161e2d;background:rgba(21,99,223,0.04);' }}border-bottom:1px solid #ffffff;"
                    onmouseover="this.style.backgroundColor='rgba(255,255,255,0.03)'"
                    onmouseout="this.style.backgroundColor='{{ $notif->read_at ? 'transparent' : 'rgba(21,99,223,0.04)' }}'">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0 flex-1">
                            <p class="text-sm sm:text-xs font-semibold" style="color:{{ $notif->read_at ? '#5c6368' : '#1563df' }};">{{ $data['title'] ?? 'Notifikasi' }}</p>
                            <p class="text-xs mt-0.5 leading-relaxed break-words" style="color:#a3abb0;">{{ $data['message'] ?? '' }}</p>
                            <p class="text-xs mt-1" style="color:#a3abb0;">{{ $notif->created_at->diffForHumans() }}</p>
                        </div>
                        @if(!$notif->read_at)
                            <span class="w-2 h-2 rounded-full shrink-0 mt-1.5" style="background:#1563df;"></span>
                        @endif
                    </div>
                </button>
            @empty
                @if($this->pendingPayCount === 0)
                    <div class="flex flex-col items-center justify-center h-full px-4 py-16 text-center">
                        <svg class="w-12 h-12 mb-3 opacity-20" fill="none" viewBox="0 0 24 24" stroke="#1563df"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        <p class="text-sm" style="color:#a3abb0;">Belum ada notifikasi</p>
                    </div>
                @endif
            @endforelse
        </div>
